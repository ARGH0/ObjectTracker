...
internal static class UsbCameraDeviceEnumerator
{
    ...
    internal static IReadOnlyList<UsbCameraDevice> EnumerateWindowsDevices(
        Func<int, bool>? probeDevice = null,
        IEnumerable<int>? candidateIndices = null)
    {
        if (!OperatingSystem.IsWindows())
        {
            return Array.Empty<UsbCameraDevice>();
        ...
        var probe = probeDevice ?? ProbeWindowsCamera;
        var devices = (candidateIndices ?? Enumerable.Range(0, 8))
            .Where(index => probe(index))
            .Select(index => new UsbCameraDevice(
                $"usb:camera:{index}",
                $"Camera {index}",
                $"Camera {index}",
                index,
    ...
    private static bool ProbeWindowsCamera(int index)
    {
        try
        {
